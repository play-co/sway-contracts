/**
    ...
    - A SwayMoment.Collection capability that allows their sale to withdraw
    ...
    will receive the entirety of the purchase. SwayMoment will initialize sales
    for users with the SwayMoment admin vault as the vault where cuts get
    deposited to.
**/
...
pub contract SwayMomentMarket {

    // -----------------------------------------------------------------------
    // SwayMoment Market contract Event definitions
    // -----------------------------------------------------------------------

    /// emitted when a SwayMoment moment is listed for sale
    ...
    /// holds a SwayMoment Collection resource to store the moments that are for sale.
    ...
    // TODO: what "Market" is this? MarketSwayMoment.cdc?
    pub resource SaleCollection: Market.SalePublic {

        /// A collection of the moments that the user has for sale
        access(self) var ownerCollection: Capability<&SwayMoment.Collection>
        ...
        init (ownerCollection: Capability<&SwayMoment.Collection>,
        ...
        ///
        /// Returns: @SwayMoment.NFT: the purchased NFT
        pub fun purchase(tokenID: UInt64, buyTokens: @DapperUtilityCoin.Vault): @SwayMoment.NFT {
            ...
            if self.prices[tokenID] != nil {
                ...
                let boughtMoment <- self.ownerCollection.borrow()!.withdraw(withdrawID: tokenID) as! @SwayMoment.NFT
        ...
        ///
        /// Returns: &SwayMoment.NFT? Optional reference to a moment for sale
    ...
    pub fun createSaleCollection(ownerCollection: Capability<&SwayMoment.Collection>,
                                 ownerCapability: Capability<&{FungibleToken.Receiver}>,
                                 beneficiaryCapability: Capability<&{FungibleToken.Receiver}>,
    ...
    init() {
        self.marketStoragePath = /storage/swaymomentSaleCollection
        self.marketPublicPath = /public/swaymomentSaleCollection
    }
}
